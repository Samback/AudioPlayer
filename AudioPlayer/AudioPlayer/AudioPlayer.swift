...
private class ClosureContainer: NSObject {
    ...
    dynamic func callSelectorOnTarget(_ sender: AnyObject) {
        closure(sender)
    }
}
...
open class AudioPlayer: NSObject {
    ...
    public override init() {
        ...
        observe(ReachabilityChangedNotification.rawValue, selector: #selector(reachabilityStatusChanged(_:)), object: reachability)
    ...
    fileprivate var player: AVPlayer? {
        didSet {
            ...
            if let player = player {
                ...
                #if os(iOS) || os(tvOS)
                    observe(NSNotification.Name.AVAudioSessionInterruption.rawValue, selector: #selector(audioSessionGotInterrupted(_:)))
                    observe(NSNotification.Name.AVAudioSessionRouteChange.rawValue, selector: #selector(audioSessionRouteChanged(_:)))
                    observe(NSNotification.Name.AVAudioSessionMediaServicesWereLost.rawValue, selector: #selector(audioSessionMessedUp(_:)))
                    observe(NSNotification.Name.AVAudioSessionMediaServicesWereReset.rawValue, selector: #selector(audioSessionMessedUp(_:)))
                #endif
                observe(NSNotification.Name.AVPlayerItemDidPlayToEndTime.rawValue, selector: #selector(playerItemDidEnd(_:)))
            }
        }
    }
    ...
    fileprivate func createAVPlayer(_ URLInfo: AudioItemURL) -> AVPlayer {
       return AVPlayer(playerItem: self.createAVPlayerItem(URLInfo))
    }
    
    fileprivate func createAVPlayerItem(_ URLInfo: AudioItemURL) -> AVPlayerItem {
    ...
    */
    open func playItems(_ items: [AudioItem], startAtIndex index: Int = 0) {
        if items.count > 0 {
            ...
            var idx = 0
            enqueuedItems = items.map { item in
                idx += 1
                return (position: idx, item: item)
            }
    ...
    */
    open func addItemsToQueue(_ items: [AudioItem]) {
        if currentItem != nil {
            ...
            var toAdd = items.map { item -> (position: Int, item: (AudioItem)) in
                idx += 1
                return (position: idx, item: item)
            }
    ...
    */
    ...
    @objc fileprivate func reachabilityStatusChanged(_ note: Notification) {
        if state == .waitingForConnection {
            if let connectionLossDate = connectionLossDate, reachability!.isReachable {
    ...
    */
    ...
    fileprivate func urlInfoAudioQualityDecrease(_ isDecreasing: Bool, quality: AudioQuality) -> AudioItemURL? {
        switch quality {
        case .high:
    ...
    fileprivate func updateAudioQuality(_ currentQuality: AudioQuality) {
    ...
    */
    ...
    fileprivate func createNewTimerWithTarget(_ target: AnyObject, timeInterVal: TimeInterval) -> Timer {
        return Timer(
